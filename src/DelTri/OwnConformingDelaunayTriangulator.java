...
import org.locationtech.jts.triangulate.MidpointSplitPointFinder;
 ...
 * <pre>
 * 	 OwnConformingDelaunayTriangulator cdt = new OwnConformingDelaunayTriangulator(sites, tolerance);
 * 
 *   // optional	
 ...
 * @author Martin Davis
 */
public class OwnConformingDelaunayTriangulator 
{
	...
	// ------- OTHER SplitPointFinders:
	private ConstraintSplitPointFinder splitFinder = new MidpointSplitPointFinder();  
	//private ConstraintSplitPointFinder splitFinder = new EqualDistanceSplitPointFinder();
	
	private KdTree kdt = null;
	private OwnConstraintVertexFactory vertexFactory = null;
	 ...
	 *          a collection of {@link OwnConstraintVertex}
	 * @param tolerance
	 *          the distance tolerance below which points are considered identical
	 */
	public OwnConformingDelaunayTriangulator(Collection initialVertices,
			double tolerance) {
	 ...
	 * The unique set of vertices (as {@link OwnConstraintVertex}es) 
	 ...
	 * @param segVertices the set of unique {@link OwnConstraintVertex}es referenced by the segments
	 */
	 ...
	 */
	public OwnConstraintVertexFactory getVertexFactory() {
	 ...
	 */
	public void setVertexFactory(OwnConstraintVertexFactory vertexFactory) {
	...
	private OwnConstraintVertex createVertex(Coordinate p) {
		OwnConstraintVertex v = null;
		if (vertexFactory != null)
			v = vertexFactory.createVertex(p, null);
		else
			v = new OwnConstraintVertex(p);
		return v;
	}
	 ...
	 */
	private OwnConstraintVertex createVertex(Coordinate p, Segment seg) {
		OwnConstraintVertex v = null;
		if (vertexFactory != null)
	 ...
	 * 
	 * @param vertices a collection of OwnConstraintVertex
	 */
	private void insertSites(Collection vertices) {
		Debug.println("Adding sites: " + vertices.size());
		for (Iterator i = vertices.iterator(); i.hasNext();) {
			OwnConstraintVertex v = (OwnConstraintVertex) i.next();
			insertSite(v);
		}
	}

	private OwnConstraintVertex insertSite(OwnConstraintVertex v) {
		KdNode kdnode = kdt.insert(v.getCoordinate(), v);
		if (!kdnode.isRepeated()) {
			incDel.insertSite(v);
		} else {
			OwnConstraintVertex snappedV = (OwnConstraintVertex) kdnode.getData();
			snappedV.merge(v);
			return snappedV;
			// testing
	...
	private final static int MAX_SPLIT_ITER = 100;
	...
	private int enforceGabriel(Collection segsToInsert) {
		...
		for (Iterator i = segsToInsert.iterator(); i.hasNext();) {
			...
			OwnConstraintVertex splitVertex = createVertex(splitPt, seg);
			 ...
			 * </ul>
			 */
			OwnConstraintVertex insertedVertex = insertSite(splitVertex);
