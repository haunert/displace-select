 ...
 *
 */
public class OwnConformingDelaunayTriangulationBuilder 
{
	...
	public OwnConformingDelaunayTriangulationBuilder()
	{
	}
   ...
   * The constraints must not contain duplicate segments (up to orientation).
	...
	private List createSiteVertices(Collection coords)
	{
		...
		for (Iterator i = coords.iterator(); i.hasNext(); ) {
			...
			if (constraintVertexMap.containsKey(coord)) 
			  continue;
			verts.add(new OwnConstraintVertex(coord));
		}
		return verts;
	}
	...
	private void createVertices(Geometry geom)
	{
		...
		for (int i = 0; i < coords.length; i++) {
			Vertex v = new OwnConstraintVertex(coords[i]);
